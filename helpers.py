...
def explore_3D_of_3arrays(arr1: np.ndarray, arr2: np.ndarray, arr3: np.ndarray, cmap: str = 'gray'):

  max = arr1.shape[0] - 1
  if arr2.shape[0] - 1 < max:
    max = arr2.shape[0] - 1
  if arr3.shape[0] - 1 < max:
    max = arr3.shape[0] - 1

  def fn(slice_index: int):
    fig, axes = plt.subplots(nrows=1, ncols=3, figsize=(15, 5))  # Increased width to accommodate three columns
    
    axes[0].imshow(arr1[slice_index, :, :], cmap=cmap)
    axes[0].axis('off')
    axes[0].set_title('ASR')
    
    axes[1].imshow(arr2[slice_index, :, :], cmap=cmap)
    axes[1].axis('off')
    axes[1].set_title('IAL')
    
    axes[2].imshow(arr3[slice_index, :, :], cmap=cmap)
    axes[2].axis('off')
    axes[2].set_title('RSP')
      
    plt.tight_layout()  
    plt.show()

  slice_slider = IntSlider(value=0, min=0, max=max, step=1, description='Slice index:', continuous_update=False)
  interact(fn, slice_index=slice_slider)
